  ...
  return (
    <>
      <SettingSection
        ...
        description="Decide which actions move freely, pause for approval, or stay out of reach. Each new run freezes the policy it starts with."
      >
        <SettingRow
          title="Permission mode"
          description="Choose a safe preset or configure each tool individually."
          layout="wide"
        >
          {permissionModeSelector}
          {permissionMode !== "custom" ? (
            <p className="mt-3 text-xs leading-5 text-muted-foreground">
              The preset applies to the next run. Assigned integrations and
              account scopes do not change.
            </p>
          ) : null}
        </SettingRow>
        <SettingRow
          title="Tool rules"
          description="Inspect effective access by capability. Select Custom to edit individual tools."
          layout="wide"
        >
          <div className="mb-3 flex flex-wrap gap-x-3 gap-y-1 text-[0.68rem] font-medium text-muted-foreground">
                  ...
                  <summary className="flex min-h-14 cursor-pointer list-none items-center gap-3 px-3 py-2.5 marker:hidden sm:px-4 [&::-webkit-details-marker]:hidden">
                    ...
                    {isSaving ? (
                      ...
                      <span className="hidden font-mono text-[0.65rem] text-muted-foreground sm:block">
                        {counts.approve} allow · {counts.prompt} ask ·{" "}
                        {counts.deny} off
                      </span>
                    )}
                  </summary>
